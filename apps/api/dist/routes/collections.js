...
async function collectionSummary(collection, aggregate) {
    return {
...
export function createCollectionsRouter() {
    ...
    router.get('/', asyncHandler(async (_request, response) => {
        ...
        const [counts, covers] = collectionIds.length === 0
            ? [[], []]
            : await Promise.all([
                ItemModel.aggregate([
                    {
        ...
        response.json({
            collections: await Promise.all(collections.map((collection) => collectionSummary(collection, aggregatesById.get(collection._id.toHexString())))),
        });
    }));
    router.get('/:collectionId/cards', asyncHandler(async (request, response) => {
        const collection = await CardCollectionModel.findById(parseObjectId(request.params.collectionId)).exec();
        if (!collection) {
    ...
    router.get('/:collectionId', asyncHandler(async (request, response) => {
        const collection = await CardCollectionModel.findById(parseObjectId(request.params.collectionId)).exec();
        if (!collection) {
            throw new AppError(404, 'collection_not_found', 'Card collection not found');
        }
    ...
    router.patch('/:collectionId', asyncHandler(async (request, response) => {
    ...
    router.delete('/:collectionId', asyncHandler(async (request, response) => {
        const collectionId = parseObjectId(request.params.collectionId);
        const collection = await CardCollectionModel.findById(collectionId).exec();
        if (!collection) {
            throw new AppError(404, 'collection_not_found', 'Card collection not found');
        ...
        if (hasCards) {
            throw new AppError(409, 'collection_not_empty', 'Move or delete the cards before deleting this collection');
        }
