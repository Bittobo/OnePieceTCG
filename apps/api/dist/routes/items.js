import { cardGradingSchema, itemInputSchema, itemKinds, itemSortOptions, } from '@one-piece-tcg/shared';
...
const listKinds = [...itemKinds, 'sealed'];
...
function validateImage(file) {
    if (file && !hasValidImageSignature(file)) {
        throw new AppError(400, 'invalid_image_content', 'The uploaded file content does not match its image type');
    }
}
async function uploadImage(imageStore, file, itemId) {
    if (!file) {
        return undefined;
    }
    validateImage(file);
...
async function resolveImageUpload(localImage, remoteImageUrl, item, tcgplayerClient) {
    if (localImage && remoteImageUrl) {
        throw new AppError(400, 'conflicting_image_sources', 'Choose either an uploaded image or the imported TCGplayer image');
    }
    if (!remoteImageUrl)
        return localImage;
    if (item.source?.provider !== 'tcgplayer') {
        throw new AppError(400, 'invalid_remote_image', 'A TCGplayer source is required to import its product image');
    }
    return tcgplayerClient.downloadImage(remoteImageUrl, item.source.productId);
}
async function cleanFailedUpload(imageStore, image, reason) {
    if (image) {
        await deleteImageOrQueue(imageStore, image.fileId, reason);
    }
}
function applyInput(document, input) {
    for (const field of typeSpecificFields) {
        document.set(field, undefined);
    }
    document.set(input);
}
async function ensureCardCollection(item) {
    if (item.kind !== 'card')
        return;
...
export function createItemsRouter(imageStore, uploadSingleImage, tcgplayerClient) {
    const router = Router();
    router.get('/', asyncHandler(async (request, response) => {
        const query = listQuerySchema.parse(request.query);
        const filter = {};
        if (query.kind === 'sealed')
            filter.kind = { $in: ['pack', 'box'] };
        else if (query.kind)
    ...
    router.get('/:itemId', asyncHandler(async (request, response) => {
    ...
    router.post('/', uploadSingleImage, asyncHandler(async (request, response) => {
        ...
        const imageUpload = await resolveImageUpload(request.file, payload.remoteImageUrl, payload.item, tcgplayerClient);
    ...
    router.patch('/:itemId', uploadSingleImage, asyncHandler(async (request, response) => {
        ...
        if (payload.removeImage && (request.file || payload.remoteImageUrl)) {
            throw new AppError(400, 'conflicting_image_change', 'Choose either a replacement image or image removal');
        }
        await ensureCardCollection(payload.item);
        ...
        if (oldImage && (replacementImage || payload.removeImage)) {
            imageCleanupPending = await deleteImageOrQueue(imageStore, oldImage.fileId, 'replaced_or_removed_image');
        }
        response.json({
    ...
    router.patch('/:itemId/grading', asyncHandler(async (request, response) => {
        const document = await ItemModel.findById(parseObjectId(request.params.itemId)).exec();
        if (!document) {
            throw new AppError(404, 'item_not_found', 'Collection item not found');
        }
    ...
    router.patch('/:itemId/collection', asyncHandler(async (request, response) => {
        const itemId = parseObjectId(request.params.itemId);
    ...
    router.delete('/:itemId', asyncHandler(async (request, response) => {
        const document = await ItemModel.findById(parseObjectId(request.params.itemId)).exec();
        if (!document) {
            throw new AppError(404, 'item_not_found', 'Collection item not found');
        }
