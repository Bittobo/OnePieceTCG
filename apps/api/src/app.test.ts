...
describe("collection API", () => {
  ...
  beforeAll(async () => {
    ...
    const imageStore = new GridFsImageStore(
      getDatabase(),
      config.gridFsBucketName,
    );
  ...
  afterEach(async () => {
    await Promise.all([
      ...
      CardCollectionModel.deleteMany({}),
    ]);
    const database = getDatabase();
    const collections = (await database.collections()).filter((collection) =>
      collection.collectionName.startsWith(`${config.gridFsBucketName}.`),
    );
  ...
  it("creates, lists, streams, and deletes an item with a GridFS image", async () => {
    ...
    const listResponse = await request(server)
      .get("/api/items?kind=card&search=Zoro")
      .expect(200);
    ...
    const updateResponse = await request(server)
      ...
      .field(
        "payload",
        JSON.stringify({ item: { ...cardWithCollection, quantity: 4 } }),
      )
      .expect(200);
    expect(updateResponse.body.item.quantity).toBe(4);
    expect(updateResponse.body.item.image.fileId).toBe(
      createResponse.body.item.image.fileId,
    );
    ...
    await request(server)
      .delete(`/api/items/${createResponse.body.item.id}`)
      .expect(200);
